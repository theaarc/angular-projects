...
export class LanguageComponent implements OnInit {
  ...
  addcomp()
  {
    ...
    if(title.value.length != 0 && pert.value.length != 0)
    {
      ...
      this.afs.collection('nusers').doc(this.userState.id).collection('langages').doc(id)
      .set({
        id:id,
        nom:title.value,
        niveau:p,
  ...
  modifycomp()
  {
   ...
   if(title.value)
   {
    this.afs.collection('nusers').doc(this.userState.id).collection('langages').doc(this.ucompid).update({nom:title.value})
   }

    if(pert.value)
    {
      let p : number = +pert.value
      this.afs.collection('nusers').doc(this.userState.id).collection('langages').doc(this.ucompid).update({niveau:p})
    }
  ...
  deletecomp()
  {
    this.afs.collection('nusers').doc(this.userState.id).collection('langages').doc(this.ucompid).delete()
  }
